...
배열을 정의하고 `in` 연산자로 포함 여부를 확인할 수 있습니다.

| A열 |
|-----|
| {% set common_types = ["A", "C", "D"] %} |
| {% if current_type in common_types %} |
| 공통 타입입니다 |
| {% else %} |
| 특수 타입입니다 |
| {% endif %} |
| ----- |
| {% if current_type in ["A", "C", "D"] %} |
| 공통 타입입니다 |
| {% endif %} |

`current_type`이 "A", "C", "D" 중 하나면 "공통 타입입니다"가 출력됩니다.
...
동일한 변수를 여러 번 set하면 마지막 값으로 덮어쓰게 됩니다.
...
위 예시에서 첫 번째 출력은 `original_name`의 값, 두 번째 출력은 "변경된 값"이 됩니다.
...
for 루프 내에서 병합된 셀이 있는 경우, 반복 시 병합도 함께 복제됩니다.
...
for 루프 본문 내에 병합된 셀이 있으면, 각 반복마다 동일한 병합 구조가 복제됩니다.
...
| ↑ (A와 병합) | {{ item.description }} | |
| {% endfor %} | | |

위 템플릿에서 A열 2-3행이 수직 병합되어 있다면:
...
|-----|-----|-----|
| {% for item in items %} | | |
| {{ item.title }} (A-B 병합) | ← | {{ item.price }} |
| {% endfor %} | | |

### 여러 변수가 있는 병합 셀

병합된 셀 안에 여러 변수가 있어도 정상 동작합니다.
...
제어문(`{% for %}`, `{% endfor %}`, `{% if %}` 등)이 있는 행은 병합에 포함될 수 없습니다.

| 허용 | 불가 |
|------|------|
| 데이터 행끼리 병합 | 제어문 행 + 데이터 행 병합 |

**에러 예시:**

```
TemplateRenderError: 시트 'Sheet1'에서 병합 범위 A1:A2가 
제어문 행([1])을 포함합니다. 제어문 행을 포함하는 병합은 지원하지 않습니다.
```

### 예시

`examples/merged_cells_template.xlsx`와 `examples/merged_cells_usage.py`를 참고하세요.
...
{% include_section 시트변수 "섹션이름" %}
```

**변수 사용:** 시트명을 변수로 지정할 수 있습니다. 변수는 따옴표 없이 작성합니다.

| A열 |
|-----|
| {% set target = "컴포넌트" %} |
| {% include_section target "헤더" %} |
...
5. **병합 셀과 제어문**: 병합 범위가 제어문 행을 포함할 수 없음 (데이터 행끼리만 병합 가능)
